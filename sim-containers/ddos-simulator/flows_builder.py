...
DDOS_STATS_FILE = "data/ddos_stats.csv"

# --- PART 1: Process DDoS Stats (Optional Logging) ---
if os.path.exists(DDOS_STATS_FILE):
    try:
        df_stats = pd.read_csv(DDOS_STATS_FILE)
        print(f"[+] DDoS Attack Stats: {df_stats.to_dict(orient='records')[0]}")
...
if os.path.exists(RAW_NET_FILE):
    try:
        ...
        print(f"[+] Network flows with 77 features (including Label) saved to {OUT_NET_FILE}")
