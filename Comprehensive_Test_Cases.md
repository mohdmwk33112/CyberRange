...
| **FE-Admin-02** | **Access Admin - Unauthorized (Fail)** | Verify non-admins and guests are blocked. | Role: 'user' or Guest | None | 1. Attempt to access `/admin` while logged out.<br>2. Log in as regular user and access `/admin`. | 1. Redirected to /auth/login (Guest).<br>2. Redirected to /dashboard (Student) with "Access Denied" toast. | Not tested |
...
| **FE-Admin-05** | **List Users & Expand Details** | Verify admin can see and expand user details. | Multiple users exist. | None | 1. Click "User Management" tab.<br>2. Observe user table.<br>3. Click on a user row (not actions). | User table loads.<br>Clicking row rotates chevron and reveals User ID and Joined Date below the row. | Not tested |
| **FE-Admin-06** | **Delete User** | Verify admin can delete a user. | Target user exists. | Click "Delete" | 1. Locate user in list.<br>2. Click "Delete".<br>3. Observe custom confirmation dialog.<br>4. Click "Delete User" button in dialog. | User removed from list.<br>Success toast appears.<br>Dialog closes. | Not tested |
